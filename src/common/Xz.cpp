...
bool Xz::compress(const char* inpath, const char* outpath){
	...
	compress(infile, outpath);
	fclose(infile);
}

bool Xz::compress(FILE* infile, const char* outpath){
	FILE* outfile=fopen( outpath, "wb");
	compress(infile, outfile);
	fclose( outfile );
}

bool Xz::compress(FILE* infile, FILE* outfile){
	init_encoder();
...
bool Xz::decompress(const char* inpath, const char* outpath){
	FILE* infile=fopen( inpath, "rb");
	decompress(infile, outpath);
	fclose(infile);
}

bool Xz::decompress(FILE* infile, const char* outpath){
	FILE* outfile=fopen( outpath, "wb");
	decompress(infile, outfile);
	fclose( outfile );
}

bool Xz::decompress( FILE* infile, FILE* outfile ){
	init_decoder();
